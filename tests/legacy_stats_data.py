...
Verbatim copy of stats_data.py as of commit c972c6c, the last commit before the
polars lazy-scanning migration (task-40). Used by test_pandas_polars_parity.py
to run the old pandas calculations side by side with the new polars ones on
identical input and confirm the migration didn't change report output.
"""
