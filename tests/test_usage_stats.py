...
class TestFilterFunctions:
    ...
    def test_filter_priority_unclaimed(self, sample_gpu_data):
        ...
        # Should have 2 unclaimed priority slots (slot2 at 10:00, slot1 at 10:15)
        assert len(result) == 2
        assert all(result["State"] == "Unclaimed")
        assert all(result["PrioritizedProjects"] != "")
...
class TestCalculationFunctions:
    ...
    def test_calculate_time_series_usage(self, sample_gpu_data):
        ...
        expected_cols = [
            ...
            "priority_claimed",
            "priority_total",
            "priority_usage_percent",
            "shared_claimed",
            "shared_total",
            "shared_usage_percent",
            "backfill_claimed",
            "backfill_total",
            "backfill_usage_percent",
        ]
...
class TestIntegrationFunctions:
    ...
    def test_run_analysis_allocation(self, temp_db_with_data):
        ...
        assert (
            "Backfill-OpenCapacity" in allocation_stats
            or "Backfill-ResearcherOwned" in allocation_stats
            or "Backfill-CHTCOwned" in allocation_stats
        )
    ...
    def test_run_analysis_device_grouping(self, temp_db_with_data):
        ...
        priority_devices = device_stats.get("Priority-ResearcherOwned", {}) or device_stats.get(
            "Priority-CHTCOwned", {}
        )
