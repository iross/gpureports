#!/usr/bin/env python3
"""
GPU slot classification and dedup
...
remain pandas-based and are consumed by standalone scripts -- filter_df/
filter_df_enhanced are their classification logic, folded in from the former
gpu_utils.py since both are the same "raw rows -> classified slots" job, just
on different DataFrame backends.
"""
...
# Host-exclusion state consumed by filter_df/filter_df_enhanced/prepare_frames.
# Set externally, e.g. HOST_EXCLUSIONS = read_data.load_host_exclusions(...).
HOST_EXCLUSIONS: dict[str, str] = {}
FILTERED_HOSTS_INFO: list[dict] = []
...
def prepare_frames(lf: pl.LazyFrame, bucket_minutes: int = 15) -> PreparedFrames:
    ...
    The duplicate-slot ranking (see _apply_duplicate_cleanup) is
