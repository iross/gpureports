...
from gpu_utils import count_backfill, count_prioritized, count_shared, filter_df
...
def get_binned_usage(df, utilization, host="", group_by=""):
    ...
    if group_by == "":
        df = (
            df.groupby(["15min_bucket", "AssignedGPUs", "State", "Name", "PrioritizedProjects"])
            .size()
        ...
        df = (
            df.groupby(["15min_bucket", "AssignedGPUs", "State", "Name", "PrioritizedProjects", group_by])
            .size()
            .reset_index(name="count")
        )
        for bucket in df["15min_bucket"].unique():
            ...
            for group_value in df_bucket[group_by].unique():
                if "GTX 1080" in group_value:
                    continue
                if "A30" in group_value:
                    continue
                if "A40" in group_value:
                    continue
                if "P100" in group_value:
                    continue
                if "Quadro" in group_value:
                    continue
...
def plot_binned_usage(prio_df, shared_df, backfill_df, name=""):
    ...
    if not has_groups:
        ...
        backfill_df["usage"] = backfill_df.apply(
            lambda row: row["used"] / row["total"] if row["total"] > 0 else 0, axis=1
        )
        print(prio_df)
        print(shared_df)
        print(backfill_df)
        ...
        for i, group in enumerate(all_groups):
            ...
            if not prio_group.empty:
                prio_group["usage"] = prio_group.apply(
                    lambda row: row["used"] / row["total"] if row["total"] > 0 else 0, axis=1
                )
                ax.plot(
                    prio_group["timestamp"],
                    prio_group["usage"],
                    "b-",
                    linewidth=2,
                    label=f"Priority {str(prio_group['total'].max())} peak",
                )

            if not shared_group.empty:
                shared_group["usage"] = shared_group.apply(
                    lambda row: row["used"] / row["total"] if row["total"] > 0 else 0, axis=1
                )
                ax.plot(
                    shared_group["timestamp"],
                    shared_group["usage"],
                    "g-",
                    linewidth=2,
                    label=f"Shared {str(shared_group['total'].max())} peak",
                )

            if not backfill_group.empty:
                backfill_group["usage"] = backfill_group.apply(
                    lambda row: row["used"] / row["total"] if row["total"] > 0 else 0, axis=1
                )
                ax.plot(
                    backfill_group["timestamp"],
                    backfill_group["usage"],
                    "r-",
                    linewidth=2,
                    label=f"Backfill {str(backfill_group['total'].max())} peak",
                )
