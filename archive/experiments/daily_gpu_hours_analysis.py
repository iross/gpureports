...
def analyze_gpu_usage(db_files):
    ...
    all_data = defaultdict(lambda: {
        'claimed_gpus': 0, 'gpu_hours': 0, 'db_count': 0,
        'primary_gpu_hours': 0, 'primary_claimed_gpus': 0,
        'backfill_gpu_hours': 0, 'backfill_claimed_gpus': 0
    })
...
def calculate_monthly_stats(daily_data):
    ...
    monthly = defaultdict(lambda: {
        'total_hours': 0, 'days': 0, 'hours_list': [],
        'primary_hours': 0, 'backfill_hours': 0
    })
...
def format_trend_stats(trend_stats, data_name="GPU Hours"):
    ...
    return (f"Linear Trend: {direction} at {abs(trend_stats['trend_per_day']):.1f} {data_name.lower()}/day "
...
def aggregate_to_weekly(daily_data):
    ...
    weekly_data = defaultdict(lambda: {
        'dates': [], 'gpu_hours': [], 'primary_hours': [], 'backfill_hours': [],
        'claimed_gpus': [], 'primary_gpus': [], 'backfill_gpus': []
    })
    ...
    for week_start in sorted(weekly_data.keys()):
        ...
        if week_info['primary_hours']:  # Has slot breakdown
            ...
            weekly_summary.append((
                week_start, avg_claimed_gpus, total_gpu_hours,
                total_primary_hours, avg_primary_gpus,
                total_backfill_hours, avg_backfill_gpus
            ))
        else:
...
def print_summary_stats(daily_data):
    ...
    for month in sorted(monthly_stats.keys()):
        ...
        if 'primary_hours' in stats and 'backfill_hours' in stats:
            print(f"        Primary: {stats['primary_hours']:,.0f} hours, "
...
def create_plots(daily_data, output_dir=None, show_linear_trend=False, transition_date=None):
    ...
    if has_slot_breakdown:
        ...
        if transition_date:
            ...
            backfill_before = [hours for date, hours in zip(dates, backfill_hours) if date < transition_dt]
            backfill_after = [hours for date, hours in zip(dates, backfill_hours) if date >= transition_dt]
        else:
    ...
    if has_slot_breakdown:
        ...
        if show_linear_trend:
            ...
            if primary_trend:
                ax1.plot(dates, primary_trend['trend_line'], color='#2E86AB', linestyle=':', linewidth=2, alpha=0.8, label=f'Primary Trend (R²={primary_trend["r_squared"]:.3f})')
            if backfill_trend:
                ax1.plot(dates, backfill_trend['trend_line'], color='#A23B72', linestyle=':', linewidth=2, alpha=0.8, label=f'Backfill Trend (R²={backfill_trend["r_squared"]:.3f})')
            if total_trend:
                ax1.plot(dates, total_trend['trend_line'], color='#404040', linestyle=':', linewidth=2, alpha=0.8, label=f'Total Trend (R²={total_trend["r_squared"]:.3f})')
        ...
        if show_linear_trend:
            ...
            total_trend = calculate_linear_trend(daily_dates, daily_total)
            if total_trend:
                ax1.plot(dates, total_trend['trend_line'], color='#C9302C', linestyle=':', linewidth=2, alpha=0.8, label=f'Linear Trend (R²={total_trend["r_squared"]:.3f})')
                ax1.legend()
    ...
    if has_slot_breakdown:
        ...
        ax2.fill_between(dates, primary_hours, [p+b for p, b in zip(primary_hours, backfill_hours)],
                        alpha=0.8, color='#A23B72', label='Backfill Slots')
        ax2.legend()
    ...
    if has_slot_breakdown:
        if transition_date and backfill_before and backfill_after:
            ...
            ax3.hist([primary_hours, backfill_before, backfill_after], bins=20, alpha=0.7,
                    label=['Primary Slots', f'Backfill (Before {transition_date})', f'Backfill (After {transition_date})'],
                    color=['#2E86AB', '#A23B72', '#F18F01'], edgecolor='#404040')
            ...
            ax3.hist([primary_hours, backfill_hours], bins=20, alpha=0.8,
                    label=['Primary Slots', 'Backfill Slots'],
                    color=['#2E86AB', '#A23B72'], edgecolor='#404040')
            ax3.legend()
    ...
    if has_slot_breakdown:
        ...
        width = 0.35
        x = range(len(months))
        ax4.bar([i - width/2 for i in x], primary_monthly, width, label='Primary Slots',
                color='#2E86AB', alpha=0.8, edgecolor='#404040')
        ax4.bar([i + width/2 for i in x], backfill_monthly, width, label='Backfill Slots',
                color='#A23B72', alpha=0.8, edgecolor='#404040')
        ax4.set_xticks(x)
    ...
    if has_slot_breakdown:
        ...
        if show_linear_trend:
            ...
            if primary_trend and dates:
                ...
                primary_trend_values = [primary_trend['slope'] * x + primary_trend['intercept'] for x in weekly_date_nums]
                plt.plot(dates, primary_trend_values, color='#2E86AB', linestyle='--', linewidth=3, alpha=0.9, label=f'Primary Linear Trend (R²={primary_trend["r_squared"]:.3f})')
            if backfill_trend and dates:
                weekly_date_nums = [datetime.strptime(row[0], '%Y-%m-%d').toordinal() for row in daily_data]
                backfill_trend_values = [backfill_trend['slope'] * x + backfill_trend['intercept'] for x in weekly_date_nums]
                plt.plot(dates, backfill_trend_values, color='#A23B72', linestyle='--', linewidth=3, alpha=0.9, label=f'Backfill Linear Trend (R²={backfill_trend["r_squared"]:.3f})')
            if total_trend and dates:
                weekly_date_nums = [datetime.strptime(row[0], '%Y-%m-%d').toordinal() for row in daily_data]
                total_trend_values = [total_trend['slope'] * x + total_trend['intercept'] for x in weekly_date_nums]
                plt.plot(dates, total_trend_values, color='#404040', linestyle='--', linewidth=3, alpha=0.9, label=f'Total Linear Trend (R²={total_trend["r_squared"]:.3f})')
        ...
        if show_linear_trend:
            ...
            if total_trend and dates:
                weekly_date_nums = [datetime.strptime(row[0], '%Y-%m-%d').toordinal() for row in daily_data]
                total_trend_values = [total_trend['slope'] * x + total_trend['intercept'] for x in weekly_date_nums]
                plt.plot(dates, total_trend_values, color='#404040', linestyle='--', linewidth=3, alpha=0.9, label=f'Linear Trend (R²={total_trend["r_squared"]:.3f})')
...
def main():
    ...
    parser.add_argument('--databases', '-d', nargs='+',
                       default=['gpu_state_2025-06.db', 'gpu_state_2025-07.db',
                               'gpu_state_2025-08.db', 'gpu_state_2025-09.db', 'gpu_state_2025-10.db'],
                       help='Database files to analyze')
    parser.add_argument('--output', '-o', help='Output CSV file path')
    parser.add_argument('--detailed', action='store_true',
                       help='Show detailed daily breakdown')
    parser.add_argument('--plot', action='store_true',
                       help='Generate and save visualization plots as PNG')
    parser.add_argument('--trend', action='store_true',
                       help='Generate and save trend analysis plot with moving average as PNG')
    parser.add_argument('--linear-trend', action='store_true',
    ...
    parser.add_argument('--transition-date', default=None,
    ...
    if args.detailed:
        ...
        if len(daily_summary[0]) > 5:  # Has slot breakdown
            ...
            for row in daily_summary:
                ...
                print(f"{date_str:<12} {claimed_gpus:<11.1f} {gpu_hours:<12.0f} {primary_hours:<13.0f} {backfill_hours:<14.0f}")
        else:
    ...
    if args.output:
        with open(args.output, 'w', newline='') as csvfile:
            ...
            if len(daily_summary[0]) > 5:  # Has slot breakdown
                writer.writerow(['date', 'claimed_gpus', 'gpu_hours', 'primary_gpu_hours',
                               'primary_claimed_gpus', 'backfill_gpu_hours', 'backfill_claimed_gpus'])
            else:
