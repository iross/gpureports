...
from gpu_utils import get_required_databases
...
def prepare_timeline_data(df: pd.DataFrame, bucket_minutes: int = 5) -> pd.DataFrame:
    """
...
def filter_gpus(
    ...
    if hostname_pattern:
        ...
        filtered_df = filtered_df[filtered_df["hostname"].apply(lambda x: bool(pattern.search(str(x))))]

    if gpu_model_pattern:
        # Filter by GPU model pattern
        pattern = re.compile(gpu_model_pattern, re.IGNORECASE)
        filtered_df = filtered_df[filtered_df["device_name"].apply(lambda x: bool(pattern.search(str(x))))]
...
def create_heatmap(
    ...
    pivot_df = timeline_df.pivot(index="gpu_identifier", columns="time_bucket", values="state")
    ...
    for state_key, label in STATE_LABELS.items():
        legend_elements.append(plt.Rectangle((0, 0), 1, 1, fc=STATE_COLORS[state_key], label=label))

    ax.legend(handles=legend_elements, loc="upper right", bbox_to_anchor=(1.15, 1), title="GPU State", fontsize=9)
...
def create_html_heatmap(timeline_df: pd.DataFrame, output_path: str, title: str = "GPU Timeline Heatmap") -> None:
    """
...
<body>
    ...
    for gpu_identifier in sorted(pivot_df.index):
        ...
        for ts in time_columns:
            ...
            hostname = (
                timeline_df[timeline_df["gpu_identifier"] == gpu_identifier]["hostname"].iloc[0]
                if not timeline_df[timeline_df["gpu_identifier"] == gpu_identifier].empty
                else "Unknown"
            )
...
def main(
    ...
    gpu_ids: str | None = typer.Option(
        None,
        help="Comma-separated list of specific GPU IDs to include (supports hex IDs like '5d6e65db' or full format 'GPU-5d6e65db')",
    ),
    host: str | None = typer.Option(None, help="Exact hostname match (e.g., 'txie-dsigpu4000.chtc.wisc.edu')"),
    hostname_pattern: str | None = typer.Option(None, help="Regex pattern for hostname filtering"),
    gpu_model_pattern: str | None = typer.Option(None, help="Regex pattern for GPU model filtering"),
    end_time: str | None = typer.Option(
        None, help="End time for analysis (YYYY-MM-DD HH:MM:SS), defaults to latest in DB"
    ),
    title: str | None = typer.Option(None, help="Custom title for the heatmap"),
    ...
    list_gpus: bool = typer.Option(
        False, help="List available GPU IDs and exit (useful for finding GPU IDs to filter)"
    ),
):
    """
    ...
    if end_time:
        try:
            ...
            print("Error: Invalid end_time format. Use YYYY-MM-DD HH:MM:S")
            return
    ...
    if title is None:
        if host:
            ...
            title = "GPU Timeline Heatmap - Selected GPUs"
        else:
    ...
    if output_format.lower() == "png":
        create_heatmap(filtered_df, str(full_output_path), title=title, figsize=(width, height))
    else:  # html
        create_html_heatmap(filtered_df, str(full_output_path), title=title)
