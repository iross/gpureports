...
from classify_slots import filter_df
from read_data import load_host_exclusions
...
def analyze_gpu_categories(db_path: str, include_all_devices: bool = False) -> dict:
    ...
    results = {"timestamp": timestamp, "total_records": len(df), "categories": {}}
        ...
        for state in ["Claimed", "Unclaimed"]:
            ...
            if not include_all_devices:
                filtered_df = filtered_df[
                    filtered_df.apply(
                        lambda row: should_include_device(row["GPUs_DeviceName"], include_all_devices), axis=1
                    )
                ]
        ...
        for state in ["Claimed", "Unclaimed"]:
            ...
            for machine in sorted(by_machine.keys()):
                ...
                for gpu in machine_gpus:
                    ...
                    gpu_usage = (
                        f"{float(gpu['gpu_usage'])*100:.1f}%"
                        if gpu["gpu_usage"] is not None and gpu["gpu_usage"] != ""
                        else "N/A"
                    )
...
def print_anomaly_analysis(results: dict):
    ...
    if backfill_only:
        ...
        print("  No GPUs are exclusively in Backfill category")
