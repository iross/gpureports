...
import gpu_utils
from gpu_timeline_heatmap import get_time_filtered_data, prepare_timeline_data, STATE_COLORS, STATE_LABELS
...
def generate_host_page(
    ...
    cmd = [
        'uv', 'run', 'python', '../gpu_timeline_heatmap.py',
        '--db-path', db_path,
...
<body>
    ...
    <div class="section">
        ...
        for host in sorted(device_hosts, key=lambda x: x['hostname']):
            ...
            utilization_color = '#28a745' if host['utilization_pct'] > 70 else '#ffc107' if host['utilization_pct'] > 30 else '#dc3545'
...
def main(
    ...
    print(f"🚀 Generating GPU monitoring website...")
    ...
    for i, host in enumerate(hosts, 1):
        ...
        html_file = generate_host_page(
            hostname, output_path, db_path, hours_back, parsed_end_time
        )
