...
from usage_stats import get_time_filtered_data, calculate_backfill_usage_by_user
from gpu_utils import (filter_df_enhanced, load_host_exclusions, 
                       analyze_backfill_utilization_by_day, BACKFILL_SLOT_TYPES)
import gpu_utils
...
def create_usage_plots(usage_df, output_dir='plots'):
    ...
    total_usage = usage_df.groupby('date').agg({
        'AssignedGPUs': 'sum',
        'State': 'sum'
    }).reset_index()
    ...
    for i, slot_type in enumerate(usage_df['slot_type'].unique()):
        ...
        ax.plot(slot_data['date'], slot_data['utilization'], 
                marker=markers[i], label=slot_type, linewidth=3, markersize=6,
                color=colors[i % len(colors)])
    
    # Add total backfill usage line
    ax.plot(total_usage['date'], total_usage['total_utilization'], 
            marker='D', label='Total Backfill', linewidth=4, markersize=7,
...
def print_summary_stats(usage_df):
    ...
    total_stats = usage_df.groupby('date').agg({
        'AssignedGPUs': 'sum',
        'State': 'sum'
    }).reset_index()
