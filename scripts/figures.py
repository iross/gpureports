...
def gpu_host_utilization(df, startd_name):
    ...
    time_range = pd.date_range(start=df["start"].min(), end=df["end"].max(), freq="H")
...
def gpu_host_gantt_chart(df, startd_name):
    ...
    ax.barh(
        df["AssignedGPUs"],
        width=pd.to_datetime(df["CompletionDate"], unit="s") - pd.to_datetime(df["JobCurrentStartDate"], unit="s"),
        left=pd.to_datetime(df["JobCurrentStartDate"], unit="s"),
        color=colors,
    )
    ax.set_xlabel("Time")
    ax.set_ylabel("GPU")
...
def gpu_model_by_prio(gpusdf):
    ...
    plot_data = pd.DataFrame({"Non-Prioritized": nonprio_counts, "Prioritized": prio_counts}).fillna(0)
